...
def multiscale_batch_iterator(single_loader,
                              ...
                              base_img_size):
    """
    Yields (images, targets) where:
        batch_size * img_size^2 ≈ base_batch_size * base_img_size^2
    """
    pixel_budget = base_batch_size * (base_img_size ** 2)

    imgs, targets = [], []
    current_size = random.choice(image_sizes)
    max_batch_for_size = max(1, pixel_budget // (current_size ** 2))
    resize_crop = transforms.RandomResizedCrop(current_size)

    for img, target in single_loader:
        # img is a PIL image from the dataset
        img = resize_crop(img)
        ...
        if len(imgs) == max_batch_for_size:
            # stack into a tensor batch and yield
            batch_imgs = torch.stack(imgs, dim=0)         # [B, C, H, W]
            batch_targets = torch.as_tensor(targets)      # [B]
            yield batch_imgs, batch_targets

            # reset and pick a new size for the next batch
            imgs, targets = [], []
            current_size = random.choice(image_sizes)
            max_batch_for_size = max(1, pixel_budget // (current_size ** 2))
            resize_crop = transforms.RandomResizedCrop(current_size)

    # tail batch at the end of the epoch
    if imgs:
        batch_imgs = torch.stack(imgs, dim=0)
        batch_targets = torch.as_tensor(targets)
        yield batch_imgs, batch_targets
...
train_loader = DataLoader(
    ...
    num_workers=args.workers,
    pin_memory=True,
    collate_fn=lambda x: x[0],  # x is a list of length 1 -> return (img, target)
)
