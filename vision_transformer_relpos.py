...
@register_model
def vit_relpos_small_patch14_dinov2(pretrained=False, patch_size=14, **kwargs) -> VisionTransformerRelPos:
    """ ViT-S/14 DINOv2 counterpart with RelPos embed
    """
    model_args = dict(patch_size=patch_size, embed_dim=384, depth=12, num_heads=6, init_values=1e-5, class_token=True, **kwargs)
    model = _create_vision_transformer_relpos(
        'vit_relpos_small_patch14_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model

@register_model
def vit_relpos_base_patch14_dinov2(pretrained=False, patch_size=14, **kwargs) -> VisionTransformerRelPos:
    """ ViT-B/14 DINOv2 counterpart with RelPos embed
    """
    model_args = dict(patch_size=patch_size, embed_dim=768, depth=12, num_heads=12, init_values=1e-5, class_token=True, **kwargs)
    model = _create_vision_transformer_relpos(
        'vit_relpos_base_patch14_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def vit_relpos_large_patch14_dinov2(pretrained=False, patch_size=14, **kwargs) -> VisionTransformerRelPos:
    """ ViT-L/14 DINOv2 counterpart with RelPos embed
    """
    model_args = dict(patch_size=patch_size, embed_dim=1024, depth=24, num_heads=16, init_values=1e-5, class_token=True, **kwargs)
    model = _create_vision_transformer_relpos(
        'vit_relpos_large_patch14_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model
