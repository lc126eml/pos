...
class Eva(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            use_sin_pos_emb: Use fixed 2D sinusoidal position embeddings
        ...
        self.pos_embed = None
        if use_sin_pos_emb:
            assert not dynamic_img_size, 'Sin-cos position embedding currently requires dynamic_img_size=False.'
            assert patch_drop_rate == 0., 'Sin-cos position embedding currently requires patch_drop_rate=0.'
            grid_h, grid_w = self.patch_embed.grid_size
            pos_embed = get_2d_sincos_pos_embed(embed_dim, (grid_h, grid_w), cls_token=not self.no_embed_class)
            self.pos_embed = nn.Parameter(pos_embed[None, ...].to(**dd), requires_grad=False)
            use_abs_pos_emb = False
        elif use_abs_pos_emb:
            self.pos_embed = nn.Parameter(torch.empty(1, num_pos_tokens, embed_dim, **dd))
    ...
    def set_input_size(
        ...
        if self.pos_embed is not None:
            if self.pos_embed.requires_grad:
                ...
                # regenerate fixed sin-cos pos embed for new grid
                grid_h, grid_w = self.patch_embed.grid_size
                pos_embed = get_2d_sincos_pos_embed(self.embed_dim, (grid_h, grid_w), cls_token=not self.no_embed_class)
                self.pos_embed = nn.Parameter(pos_embed[None, ...].to(device=self.pos_embed.device, dtype=self.pos_embed.dtype), requires_grad=False)
...
def get_2d_sincos_pos_embed(embed_dim: int, grid_size: Tuple[int, int], cls_token: bool = False) -> torch.Tensor:
    """Create 2D sin/cos positional embeddings for a HxW grid."""
    gh, gw = grid_size
...
# Restrict cfgs to sinusoidal variants to avoid conflicts with base EVA registrations.
default_cfgs = generate_default_cfgs({
    'vit_sin_small_patch16_dinov3': _cfg(input_size=(3, 256, 256), crop_pct=1.0),
    'vit_sin_base_patch16_dinov3': _cfg(input_size=(3, 256, 256), crop_pct=1.0),
    'vit_sin_large_patch16_dinov3': _cfg(input_size=(3, 256, 256), crop_pct=1.0),
})


@register_model
def vit_sin_small_patch16_dinov3(pretrained: bool = False, **kwargs) -> Eva:
    """DINOv3 S/16 with fixed 2D sinusoidal position embeddings (no AbsPos, no RoPE)."""
    if pretrained:
        raise RuntimeError('No pretrained weights are provided for vit_sin_small_patch16_dinov3.')
    model_args = dict(
        img_size=256,
        patch_size=16,
    ...
    model = _create_eva('vit_sin_small_patch16_dinov3', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def vit_sin_base_patch16_dinov3(pretrained: bool = False, **kwargs) -> Eva:
    """DINOv3 B/16 with fixed 2D sinusoidal position embeddings (no AbsPos, no RoPE)."""
    if pretrained:
        raise RuntimeError('No pretrained weights are provided for vit_sin_base_patch16_dinov3.')
    model_args = dict(
        img_size=256,
        patch_size=16,
        dynamic_img_size=False,
    ...
    model = _create_eva('vit_sin_base_patch16_dinov3', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def vit_sin_large_patch16_dinov3(pretrained: bool = False, **kwargs) -> Eva:
    """DINOv3 L/16 with fixed 2D sinusoidal position embeddings (no AbsPos, no RoPE)."""
    if pretrained:
        raise RuntimeError('No pretrained weights are provided for vit_sin_large_patch16_dinov3.')
    model_args = dict(
        img_size=256,
        patch_size=16,
        dynamic_img_size=False,
    ...
    model = _create_eva('vit_sin_large_patch16_dinov3', pretrained=pretrained, **dict(model_args, **kwargs))
    return model
