...
# TPU launch (PJRT)
# =============================================================================
SCRIPT_REV = "tpu-depth-20260126-1"
...
def main():
    ...
    try:
        ...
        valid_loader = DataLoader(valid_dataset, sampler=valid_sampler, **valid_kwargs)
    ...
    def validate(model, decoder, loader, criterion, feature_layers, max_steps=None, *, use_sliding_window=None, sw_window_size=None, sw_overlap=None):
        ...
        metrics_keys = ["abs_rel", "mae", "rmse", "a1", "a2", "a3"]
        metrics_t = torch.tensor([val_metrics[k] for k in metrics_keys], device=DEVICE)
        steps_t = torch.tensor(steps, device=DEVICE)
        if WORLD_SIZE > 1:
            xm.all_reduce(xm.REDUCE_SUM, [metrics_t, steps_t])
        denom = max(int(steps_t.item()), 1)
        out = {k: (metrics_t[i] / denom).item() for i, k in enumerate(metrics_keys)}
        return 0.0, out
    ...
    if args.train:
        ...
        for epoch in range(start_epoch, EPOCHS):
            ...
            if IS_MASTER:
                ...
                logger.info(
                    " Valid AbsRel: %.4f | Valid MAE: %.4f | Valid RMSE: %.4f | Valid a1: %.4f\n",
                    avg_val_metrics["abs_rel"], avg_val_metrics["mae"], avg_val_metrics["rmse"], avg_val_metrics["a1"],
                )
